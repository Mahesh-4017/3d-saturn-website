...
const ProjectCard: React.FC<ProjectCardProps> = ({ project, index }) => {
  ...
  return (
    <div
      ref={cardRef}
      className={`card-3d-tilt group relative p-8 sm:p-10 rounded-2xl bg-black/40 backdrop-blur-md border border-white/10 hover:border-amber-400/40 transition-all duration-1000 ease-out flex flex-col justify-between space-y-8 overflow-hidden transform ${
        inView
    ...
    >
      ...
      <div className="space-y-5">
        <div className="flex items-center justify-between">
          ...
          <a
            ...
            className="flex items-center gap-1.5 px-4 py-1.5 rounded-full bg-white/5 border border-white/20 hover:border-amber-400 hover:text-amber-300 text-xs font-mono text-white transition-all duration-300 hover:scale-105"
          >
            <span>See Live</span>
        ...
        <h3 className="font-serif italic text-3xl sm:text-4xl font-normal text-white group-hover:text-amber-300 transition-colors duration-300">
          {project.title}
        </h3>
      ...
      <div className="flex flex-wrap items-center gap-2 pt-6 border-t border-white/10">
        {project.tags.map((tag, idx) => (
          <span
            key={idx}
            className="px-3.5 py-1 rounded-full bg-white/5 border border-white/10 text-[11px] font-mono text-gray-300 group-hover:border-amber-400/30 group-hover:text-amber-300 transition-colors"
          >
            {tag}
          </span>
        ))}
...
export const WorkSection: React.FC<WorkSectionProps> = ({
  ...
  return (
    <section
      id="work"
      className="relative py-32 px-6 sm:px-12 max-w-7xl mx-auto space-y-16 select-none overflow-hidden"
    >
      {/* Section Header */}
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-8">
        ...
        {/* Filter Pill Buttons */}
        <div className="flex flex-wrap items-center gap-2 p-1.5 rounded-full bg-white/5 border border-white/15 w-fit">
          {categories.map((cat) => (
            <button
              key={cat}
